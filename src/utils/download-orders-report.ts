...
export function downloadOrdersReport(orders: StoreOrderDTO[]) {
  ...
  const tableX = 10
  const colWidths = [30, 55, 50, 35, 30] // data, cliente, peça, status, valor
  const rowHeight = 12
  ...
  doc.text(
    'A coluna de valores representa o valor total do pedido sem o desconto de 14% *',
    20,
    60,
  )
  ...
  const headers = ['Data', 'Cliente', 'Peça', 'Status', 'Valor']
  ...
  orders.forEach((order, index) => {
    ...
    const rowData = [
      ...
      order.customerName.length > 20
        ...
        : order.customerName,
      order.clotheName.length > 22
        ? order.clotheName.substring(0, 22) + '...'
        : order.clotheName,
